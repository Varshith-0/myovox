...
Package layout (subpackages under src/, imported as emg2text via package-dir):
...
Entry points (after `pip install -e .`):
    from emg2text.config import load_config, Config
    from emg2text.decode import evaluate     # WER / PER + paired bootstrap
    python -m emg2text.reproduce             # fast acoustic check (26.14 / 22.34) from cached logits
