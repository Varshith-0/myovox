/**
 * "In One Breath" — the reel. Ten short scenes that tell the whole Myovox story
 ...
 * so scrubbing through the reel reads as one continuous film.
 */
import type { Stage } from './stages'

/** A reel Manim stage — same shape as the deep dive's `act2`, top caption. */
function reel(id: string, rail: string, caption: string, alt: string, sub?: string): Stage {
  return {
    id,
    rail,
    caption,
...
export const REEL_STAGES: readonly Stage[] = [
  reel(
    'reel-spark', 'Spark',
  ...
  reel(
    'reel-fingerprints', 'Fingerprints',
...
export const REEL_STAGE_COUNT = REEL_STAGES.length
